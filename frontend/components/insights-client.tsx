        ...
        <div className="container mx-auto px-4">
            <div className="bg-white rounded-xl overflow-hidden flex flex-col md:flex-row shadow-lg border border-gray-200">
              <div className="p-8 md:p-12 md:w-1/2 flex flex-col justify-center">
                  <Badge variant="plum" className="mb-4 w-fit">Flagship Report</Badge>
                  <h2 className="text-4xl font-bold text-kendra-blue mb-6 tracking-tight">{content.featuredReport.title}</h2>
                  <p className="text-gray-700 text-lg mb-8 leading-relaxed">
                    {content.featuredReport.subtitle}
                  </p>
                  <Button size="lg" variant="plum" className="w-fit">
      ...
      <section id="blog" className="py-24 bg-white border-t border-gray-100">
        <div className="container mx-auto px-4">
          <div className="flex justify-between items-end mb-12">
            <div>
              <h2 className="text-4xl font-bold text-kendra-blue mb-2">The Blueprint</h2>
              <p className="text-gray-600">Expert commentary on the transition from generative to agentic enterprise.</p>
            </div>
          </div>
          <div className="grid md:grid-cols-2 gap-8">
            {content.blogPosts.map((post: any) => (
              <GlassCard key={post.slug} className="flex flex-col">
                ...
                <h3 className="text-2xl font-bold text-kendra-blue mb-4">{post.title}</h3>
                <p className="text-gray-600 mb-6 flex-grow">{post.excerpt}</p>
                <Link href={`/insights/${post.slug}`} className="text-kendra-plum font-semibold hover:underline">
                  Read Full Story →
                </Link>
              </GlassCard>
            ))}
          ...
          <div className="container mx-auto px-4">
            <div className="flex flex-col lg:flex-row gap-12">
                <div className="lg:w-2/3">
                  <h3 className="text-3xl md:text-4xl font-bold mb-8 flex items-center gap-4 text-kendra-blue">
                      📰
                      Newsroom
                  </h3>
                  <div className="space-y-6">
                      {content.newsEntries.map((news: any) => (
                        <motion.div
                           ...
                           viewport={{ once: true }}
                           className="group p-6 border border-gray-200 rounded-xl hover:border-kendra-blue transition-colors cursor-pointer bg-white shadow-sm"
                        >
                           <div className="flex justify-between items-start mb-4">
                              <Badge variant="outline" className="text-xs border-kendra-blue/30 text-kendra-blue bg-kendra-blue/5">News</Badge>
                              <span className="text-xs text-gray-500">{news.date}</span>
                           </div>
                           <h4 className="text-xl font-bold mb-2 group-hover:text-kendra-blue transition-colors">{news.title}</h4>
                           <p className="text-gray-600 mb-4">{news.excerpt}</p>
                           <Link href={`/insights/${news.slug}`} className="text-kendra-blue text-sm font-medium flex items-center">
                ...
                <div className="lg:w-1/3">
                  ...
                  >
                     <h4 className="text-2xl font-bold mb-4 flex items-center gap-2 text-kendra-blue">
                        🔬
                        Lab Notes
                     </h4>
                     <div className="space-y-4 mb-8">
                        {content.labNotes.map((note: any) => (
                          <Link key={note.slug} href={`/insights/${note.slug}`} className="block group">
                            ...
                            <h5 className="font-bold text-gray-800 group-hover:text-kendra-blue transition-colors">{note.title}</h5>
                          </Link>
                        ))}
                     </div>
                     <h4 className="text-xl font-bold mb-4 text-kendra-blue">Knowledge Base</h4>
                     <div className="space-y-2">
                       {content.knowledgeBase.items.map((item: any) => (
                         <div key={item} className="p-2 bg-gray-50 border border-gray-200 rounded text-xs font-medium text-gray-700 hover:text-kendra-blue cursor-pointer transition-colors">
                            {item}
                         </div>
                       ))}
                     </div>
        ...
        <div className="container mx-auto px-4 text-center">
          ...
          >
            ...
            <div className="flex justify-center">
               <Button size="lg" variant="plum">Subscribe to Updates</Button>
            </div>
          </motion.div>
        </div>
      </section>
