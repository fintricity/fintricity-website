            ...
            {content.solutionsByFunction.map((solution: any, i: number) => (
              <motion.div key={i} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.1 }}>
                <GlassCard className="flex flex-col h-full">
                  ...
                  <p className="text-gray-600 leading-relaxed mb-6 flex-grow">{solution.desc}</p>
                  <Link href={`/solutions/${solution.slug}`} className="text-kendra-blue font-bold text-sm hover:underline">
            ...
            {content.solutionsByIndustry.map((industry: any, i: number) => (
              <motion.div key={i} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.1 }}>
                <GlassCard className="flex flex-col h-full">
                  ...
                  <p className="text-gray-600 leading-relaxed mb-6 flex-grow">{industry.desc}</p>
                  <Link href={industry.slug} className="text-kendra-blue font-bold text-sm hover:underline">
                    View Case Study →
                  </Link>
                </GlassCard>
              </motion.div>
          ...
          <div className="max-w-4xl mx-auto text-center">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              ...
              <Button size="lg" variant="outline" className="border-white text-white hover:bg-white/10" asChild>
                <Link href="/">{content.economicImpact.cta.secondaryButton}</Link>
              </Button>
            </div>
          </div>
        </div>
